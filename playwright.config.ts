...
export default defineConfig({
  ...
  webServer: externalBaseURL
    ? undefined
    : {
        command: `rm -f ${quoteShell(smokeDbPath)} ${quoteShell(`${smokeDbPath}-shm`)} ${quoteShell(`${smokeDbPath}-wal`)} && DB_PATH=${quoteShell(smokeDbPath)} npm run smoke:seed && PORT=${smokePort} DB_PATH=${quoteShell(smokeDbPath)} LOGIN_RATE_LIMIT_MAX=0 COOKIE_SECURE=false npm run smoke:serve`,
        port: smokePort,
